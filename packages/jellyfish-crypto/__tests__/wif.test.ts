...
it('should decode without version', () => {
  ...
  expect(decoded.version).toStrictEqual(0xef)
  expect(decoded.compressed).toStrictEqual(true)
  expect(decoded.privateKey.toString('hex')).toStrictEqual(privKey)
})
...
it('should decode decodeAsEllipticPair', async () => {
  ...
  expect(privateKey.toString('hex')).toStrictEqual(privKey)
  expect(publicKey.toString('hex')).toStrictEqual(pubKey)
})

it('should encode', () => {
  const wif = WIF.encode(0xef, Buffer.from(privKey, 'hex'))
  expect(wif).toStrictEqual(REG_TEST_WIF)
})
