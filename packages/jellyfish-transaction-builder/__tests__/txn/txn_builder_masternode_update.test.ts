...
describe('UpdateMasternode', () => {
  ...
  it('should fail if address is P2SH', async () => {
...

    {
      ...
      await expect(promise).rejects.toThrow("DeFiDRpcError: 'UpdateMasternodeTx: Owner address must be P2PKH or P2WPKH type (code 16)', code: -26")
    }
    {
      const address = await container.getNewAddress('', 'p2sh-segwit')
      const addressDest: P2SH = P2SH.fromAddress(RegTest, address, P2SH)
      ...
      const promise = sendTransaction(container, txn)
      await expect(promise).rejects.toThrow(DeFiDRpcError)
      await expect(promise).rejects.toThrow("DeFiDRpcError: 'UpdateMasternodeTx: Operator address must be P2PKH or P2WPKH type (code 16)', code: -26")
    }
  ...
  it('should fail to update owner address without collateral transaction inputs', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'UpdateMasternodeTx: Missing previous collateral from transaction inputs (code 16)', code: -26")
  })
  ...
  it('should fail to update owner address with same address', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'UpdateMasternodeTx: Masternode with collateral address as operator or owner already exists (code 16)', code: -26")
  })
  ...
  it('should fail to update another node with operator address that is already used', async () => {
    ...
    const promise = sendTransaction(container, txn)
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'UpdateMasternodeTx: Masternode with that operator address already exists (code 16)', code: -26")
  })
  ...
  it('should not update masternode while in PRE_ENABLED or TRANSFERRING state', async () => {
...

    {
      ...
      const promise = sendTransaction(container, txn)
      await expect(promise).rejects.toThrow(DeFiDRpcError)
      await expect(promise).rejects.toThrow(`DeFiDRpcError: 'UpdateMasternodeTx: Masternode ${masternodeId} is not in 'ENABLED' state (code 16)', code: -26`)
    }
...
describe('Update Masternode (Multi-containers)', () => {
  ...
  it('should throw error if incorrect authorization is provided', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow('DeFiDRpcError: \'UpdateMasternodeTx: tx must have at least one input from the owner (code 16)\', code: -26')
  })
})
