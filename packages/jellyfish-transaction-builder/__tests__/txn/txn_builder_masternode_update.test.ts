...
describe('UpdateMasternode', () => {
  ...
  it('should fail if address is P2SH', async () => {
...

    {
      ...
      await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Owner address must be P2PKH or P2WPKH type', code: -26")
    }
    {
      const address = await container.getNewAddress('', 'p2sh-segwit')
      const addressDest: P2SH = P2SH.fromAddress(RegTest, address, P2SH)
      ...
      const promise = sendTransaction(container, txn)
      await expect(promise).rejects.toThrow(DeFiDRpcError)
      await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Operator address must be P2PKH or P2WPKH type', code: -26")
    }
    ...
    //   const promise = sendTransaction(container, txn)
    //   await expect(promise).rejects.toThrow(DeFiDRpcError)
    //   await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Reward address must be P2PKH or P2WPKH type', code: -26")
    // }
  })
  ...
  it('should be failed as invalid address is not allowed', async () => {
    ...
    //   const promise = sendTransaction(container, txn)
    //   await expect(promise).rejects.toThrow(DeFiDRpcError)
    //   await expect(promise).rejects.toThrow('DeFiDRpcError: \'bad-txns-customtx, UpdateMasternodeTx: Reward address must be P2SH, P2PKH or P2WPKH type\', code: -26')
    // }
  })
  ...
  it('should fail to update owner address without collateral transaction inputs', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Missing previous collateral from transaction inputs', code: -26")
  })
  ...
  it('should fail to update owner address with same address', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Masternode with collateral address as operator or owner already exists', code: -26")
  })
  ...
  it('should fail to update another node with operator address that is already used', async () => {
    ...
    const promise = sendTransaction(container, txn)
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow("DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Masternode with that operator address already exists', code: -26")
  })
  ...
  it('should not update masternode while in PRE_ENABLED or TRANSFERRING state', async () => {
...

    {
      ...
      const promise = sendTransaction(container, txn)
      await expect(promise).rejects.toThrow(DeFiDRpcError)
      await expect(promise).rejects.toThrow(`DeFiDRpcError: 'bad-txns-customtx, UpdateMasternodeTx: Masternode ${masternodeId} is not in 'ENABLED' state', code: -26`)
    }
...
describe('Update Masternode (Multi-containers)', () => {
  ...
  it('should throw error if incorrect authorization is provided', async () => {
    ...
    await expect(promise).rejects.toThrow(DeFiDRpcError)
    await expect(promise).rejects.toThrow('DeFiDRpcError: \'bad-txns-customtx, UpdateMasternodeTx: tx must have at least one input from the owner\', code: -26')
  })
})
