...
describe('Loan', () => {
  ...
  it('Should takeLoan', async () => {
...

    {
      ...
      // }
      // const data = await testing.rpc.poolpair.createPoolPair(metadata1)
      await container.generate(1)

      // add TSLA-dUSD
      await testing.rpc.poolpair.addPoolLiquidity({
        '*': ['20000@TSLA', '1000@DUSD']
    ...
    await testing.container.call('loanpayback', [{
      vaultId: vaultId,
      amounts: '13@TSLA',
      from: otherAddress
    }])
