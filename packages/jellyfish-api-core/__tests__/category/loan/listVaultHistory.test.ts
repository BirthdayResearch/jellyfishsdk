...
describe('Loan listVaultHistory', () => {
  ...
  describe('listVaultHistory without pagination', () => {
    ...
    it('should listAuctionHistory with vaultId', async () => {
      {
        const vaultHistory2 = (await alice.rpc.loan.listVaultHistory(vaultId2)).reverse()
        console.log(JSON.stringify(vaultHistory2))
        expect(vaultHistory2[0].type).toStrictEqual('Vault')
        expect(vaultHistory2[0].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[1].type).toStrictEqual('DepositToVault')
        expect(vaultHistory2[1].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[2].type).toStrictEqual('DepositToVault')
        expect(vaultHistory2[2].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[3].type).toStrictEqual('TakeLoan')
        expect(vaultHistory2[3].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[4].type).toStrictEqual('PaybackLoan')
        expect(vaultHistory2[4].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[5].type).toStrictEqual('PaybackLoan')
        expect(vaultHistory2[5].txid).toStrictEqual(vaultId2)

        expect(vaultHistory2[6].type).toStrictEqual('AuctionBid')
        expect(vaultHistory2[6].txid).toStrictEqual(vaultId2)
      }
    })
  })
