...
describe('Loan listVaultHistory', () => {
  // Below are the tx letters for all possible loan tx after the vault creation
  // CreateVault            = 'V'
  // DepositToVault         = 'S'
  // TakeLoan               = 'X'
  // AuctionBid             = 'I'
  ...
  let priceActivationBlockHeight: number // 11

  let closeVaultTxId: string // 12
  ...
  beforeAll(async () => {
    ...
    auctionBidTxId2 = await alice.container.call('placeauctionbid', [createVaultTxId, 1, aliceColAddr, '2500@AAPL'])
    expect(typeof auctionBidTxId2).toStrictEqual('string')
    expect(auctionBidTxId2.length).toStrictEqual(64)
    await alice.generate(1)
    auctionBidBlockHeight2 = await alice.container.getBlockCount()

    await alice.container.generate(33)
    priceActivationBlockHeight = await alice.container.getBlockCount()
  ...
  describe('listVaultHistory with vaultId', () => {
    ...
    beforeAll(async () => {
      vaultHistory = (await alice.rpc.loan.listVaultHistory(createVaultTxId)).reverse()
      validateType(vaultHistory)
    })

    it('should listVaultHistory', async () => {
      expect(vaultHistory.length).toStrictEqual(13)
      ...
      expect(vaultHistory[8].txid).toStrictEqual('') // No tx for liquidation
      ...
      expect(vaultHistory[10].amounts).toStrictEqual(['-2500.00000000@AAPL'])

      // Active price
      expect(vaultHistory[11].vaultSnapshot?.state).toStrictEqual('active')
      expect(vaultHistory[11].txid).toStrictEqual('') // No tx for price activation
      expect(vaultHistory[11].blockHeight).toStrictEqual(priceActivationBlockHeight)
      expect(vaultHistory[11].vaultSnapshot).toStrictEqual({
        state: 'active',
        collateralAmounts: [
          '28.54196807@DFI'
        ],
        collateralValue: 28.54196807,
        collateralRatio: 0
      })

      // CloseVault
      expect(vaultHistory[12].type).toStrictEqual('CloseVault')
      expect(vaultHistory[12].address).toStrictEqual(aliceColAddr)
      expect(vaultHistory[12].txid).toStrictEqual(closeVaultTxId)
      expect(vaultHistory[12].blockHeight).toStrictEqual(closeVaultBlockHeight)
      expect(vaultHistory[12].amounts).toStrictEqual(['29.04196807@DFI'])
    })
  })
  ...
  describe('listVaultHistory with pagination', () => {
    ...
    it('should listAuctionHistory with depth only', async () => {
      {
        ...
        expect(vaultHistory[0].amounts).toStrictEqual(['29.04196807@DFI'])
      }

      {
        const vaultHistory = await alice.rpc.loan.listVaultHistory(createVaultTxId, { depth: 2 })
        ...
        // Active price
        expect(vaultHistory[1].vaultSnapshot?.state).toStrictEqual('active')
        expect(vaultHistory[1].txid).toStrictEqual('') // No tx for price activation
        expect(vaultHistory[1].blockHeight).toStrictEqual(priceActivationBlockHeight)
        expect(vaultHistory[1].vaultSnapshot).toStrictEqual({
          state: 'active',
          collateralAmounts: [
            '28.54196807@DFI'
          ],
    ...
    it('should not listAuctionHistory with token only', async () => {
      {
        const vaultHistory = (await alice.rpc.loan.listVaultHistory(createVaultTxId, { token: 'AAPL' })).reverse()
        // Take loan
        expect(vaultHistory[0].type).toStrictEqual('TakeLoan')
        expect(vaultHistory[0].txid).toStrictEqual(takeLoanTxId)
        expect(vaultHistory[0].blockHeight).toStrictEqual(takeLoanBlockHeight)
        expect(vaultHistory[0].amounts).toStrictEqual(['7500.00000000@AAPL']) // TakeLoan 7500 AAPL
        ...
        expect(vaultHistory[1].amounts).toStrictEqual(['-500.00000000@AAPL'])

        // When the price is liquidated
        expect(vaultHistory[2].vaultSnapshot?.state).toStrictEqual('inLiquidation')
        expect(vaultHistory[2].txid).toStrictEqual('') // No tx for liquidation
        expect(vaultHistory[2].blockHeight).toStrictEqual(inLiquidationBlockHeight)
        expect(vaultHistory[2].vaultSnapshot?.collateralAmounts).toStrictEqual([]) // No collateralAmounts for liquidation
        expect(vaultHistory[2].vaultSnapshot?.collateralValue).toStrictEqual(0) // No collateralValue for liquidation
        expect(vaultHistory[2].vaultSnapshot?.batches).toStrictEqual([
          {
            index: 0,
            collaterals: [
              '6655.51838400@DFI',
        ...
        // Auction bid
        expect(vaultHistory[3].type).toStrictEqual('AuctionBid')
        expect(vaultHistory[3].address).toStrictEqual(aliceColAddr)
        expect(vaultHistory[3].txid).toStrictEqual(auctionBidTxId1)
        expect(vaultHistory[3].blockHeight).toStrictEqual(auctionBidBlockHeight1)
        expect(vaultHistory[3].amounts).toStrictEqual(['-5000.00000000@AAPL'])

        expect(vaultHistory[4].type).toStrictEqual('AuctionBid')
        expect(vaultHistory[4].address).toStrictEqual(aliceColAddr)
        expect(vaultHistory[4].txid).toStrictEqual(auctionBidTxId2)
        expect(vaultHistory[4].blockHeight).toStrictEqual(auctionBidBlockHeight2)
        expect(vaultHistory[4].amounts).toStrictEqual(['-2500.00000000@AAPL'])

        // Active price
        expect(vaultHistory[5].vaultSnapshot?.state).toStrictEqual('active')
        expect(vaultHistory[5].txid).toStrictEqual('') // No tx for price activation
        expect(vaultHistory[5].blockHeight).toStrictEqual(priceActivationBlockHeight)
        expect(vaultHistory[5].vaultSnapshot).toStrictEqual({
          state: 'active',
          collateralAmounts: [
            '28.54196807@DFI'
          ],
    ...
    it('should listAuctionHistory with limit only', async () => {
      ...
      {
        const vaultHistory = await alice.rpc.loan.listVaultHistory(createVaultTxId, { limit: 2 })
        // CloseVault
        expect(vaultHistory[0].type).toStrictEqual('CloseVault')
        expect(vaultHistory[0].address).toStrictEqual(aliceColAddr)
        expect(vaultHistory[0].txid).toStrictEqual(closeVaultTxId)
      ...
      // For length more than 13
      {
        const vaultHistory = await alice.rpc.loan.listVaultHistory(createVaultTxId, { limit: 15 })
        expect(vaultHistory.length).toStrictEqual(13)
      }
    })
  })
