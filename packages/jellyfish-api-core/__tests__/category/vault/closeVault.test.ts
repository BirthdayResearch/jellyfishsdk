...
describe('Vault', () => {
  ...
  async function setup (): Promise<void> {
    ...
    const mintTokenVaultId = await tGroup.get(0).rpc.vault.createVault({
    ...
    vaultWithoutCollateral1Id = await tGroup.get(0).rpc.vault.createVault({
      ownerAddress: await tGroup.get(0).generateAddress(),
      loanSchemeId: 'scheme'
    })
    await tGroup.get(0).generate(1)
    ...
    vaultWithoutCollateral2Id = await tGroup.get(0).rpc.vault.createVault({
    ...
    vaultWithoutCollateral3Id = await tGroup.get(0).rpc.vault.createVault({
      ownerAddress: await tGroup.get(0).generateAddress(),
      loanSchemeId: 'scheme'
    })
    await tGroup.get(0).generate(1)
  ...
  it('should closeVault', async () => {
    {
      ...
      const txId = await tGroup.get(0).rpc.vault.closeVault({
  ...
  it('should closeVault if loan is paid back', async () => {
    const txId = await tGroup.get(0).rpc.vault.closeVault({ vaultId: vaultWithPayBackLoanId, to: await tGroup.get(0).generateAddress() })
    expect(typeof txId).toStrictEqual('string')
    expect(txId.length).toStrictEqual(64)
    await tGroup.get(0).generate(1)
  })
  ...
  it('should not closeVault for vault with loan taken', async () => {
    const promise = tGroup.get(0).rpc.vault.closeVault({ vaultId: vaultWithLoanTakenId, to: await tGroup.get(0).generateAddress() })
  ...
  it('should not closeVault for mayliquidate vault', async () => {
    ...
    const promise = tGroup.get(0).rpc.vault.closeVault({ vaultId: vaultWithLiquidationId, to: await tGroup.get(0).generateAddress() })
  ...
  it('should not closeVault by anyone other than the vault owner', async () => {
    ...
    const promise = tGroup.get(1).rpc.vault.closeVault({ vaultId: vaultWithoutCollateral2Id, to: address })
  ...
  it('should closeVault with utxos', async () => {
    ...
    const txId = await tGroup.get(0).rpc.vault.closeVault({ vaultId: vaultWithoutCollateral2Id, to: await tGroup.get(0).generateAddress() }, [utxo])
    expect(typeof txId).toStrictEqual('string')
    expect(txId.length).toStrictEqual(64)
  ...
  it('should not closeVault with arbitrary utxos', async () => {
    ...
    const promise = tGroup.get(0).rpc.vault.closeVault({ vaultId: vaultWithoutCollateral3Id, to: await tGroup.get(0).generateAddress() }, [utxo])
