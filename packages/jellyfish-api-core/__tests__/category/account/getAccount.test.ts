...
it('should getAccount existing account with { start: undefined, including_start: true, limit: 30 }', async () => {
  const accounts = await client.account.listAccounts()
...
it('should getAccount non-existence  account with { start: undefined, including_start: true, limit: 30 }', async () => {
  const account = await client.account.getAccount('bcrt1qwm0jzcy0jfyel8he42u9536u9wzefmwr5mdkc2', {
    start: undefined,
    including_start: true,
    limit: 30
  }, { indexedAmounts: true })
