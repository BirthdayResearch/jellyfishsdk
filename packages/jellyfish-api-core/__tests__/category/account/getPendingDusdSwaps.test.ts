...
describe('getPendingDusdSwaps', () => {
  ...
  it('should getPendingDusdSwaps', async () => {
    ...
    const future = await testing.rpc.account.getPendingDusdSwaps(colAddr)
    expect(future.owner).toStrictEqual(colAddr)
    expect(future.amount).toStrictEqual(new BigNumber(74))
    ...
    // should be empty
    const futureAfter = await testing.rpc.account.getPendingDusdSwaps(colAddr)
    expect(Object.keys(futureAfter).length).toStrictEqual(0)
