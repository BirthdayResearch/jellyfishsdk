...
masternode <${masternodeId}> is not active', code: -32600, method: votegov`)
  ...
  it('should not vote on a proposal with a masternode that did not mint at least one block', async () => {
    ...
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow(`RpcApiError: 'Test VoteTx execution failed:
masternode <${masternodeId}> does not mine at least one block', code: -32600, method: votegov`)
  })
  ...
  it('should not vote on a proposal not in voting period', async () => {
    ...
    const promise = client.governance.voteGov({ proposalId, masternodeId, decision: VoteDecision.YES })
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow(`RpcApiError: 'Proposal <${proposalId}> is not in voting period', code: -8, method: votegov`)
  })
  ...
  it('should not vote with utxos not from the owner', async () => {
    ...
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow(`RpcApiError: 'Test VoteTx execution failed:
tx must have at least one input from the owner', code: -32600, method: votegov`)
  })
  ...
  it('should not vote with wrongly formatted utxos\' txid', async () => {
    ...
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow('RpcApiError: \'txid must be of length 64 (not 4, for \'XXXX\')\', code: -8, method: votegov')
  })
  ...
  it('should not vote with invalid utxos\' txid', async () => {
    ...
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow('RpcApiError: \'Insufficient funds\', code: -4, method: votegov')
  })
})
