...
describe('Governance', () => {
  ...
  it('should vote on a proposal', async () => {
    const proposalId = await tGroup.get(1).container.call('createcfp', [{
    ...
    const masternodes = await tGroup.get(1).rpc.masternode.listMasternodes()
    let masternodeId = ''
    ...
    const txid = await tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES })
    await tGroup.get(1).container.generate(1)
  ...
  it('should not vote on a proposal with a masternode that does not exist', async () => {
    ...
    const promise = tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES })
  ...
  it('should not vote on a proposal with an inactive masternode', async () => {
    ...
    const ownerAddress = await tGroup.get(1).container.getNewAddress()
    const masternodeId = await tGroup.get(1).rpc.masternode.createMasternode(ownerAddress)
    await tGroup.get(1).container.generate(1)
...
masternode <${masternodeId}> is not active', code: -32600, method: vote`)
  ...
  it('should not vote on a proposal with a masternode that did not mine at least one block', async () => {
    ...
    const ownerAddress = await tGroup.get(1).container.getNewAddress()
    const masternodeId = await tGroup.get(1).rpc.masternode.createMasternode(ownerAddress)
    await tGroup.get(1).container.generate(20) // Enables masternode
...
masternode <${masternodeId}> does not mine at least one block', code: -32600, method: vote`)
  ...
  it('should not vote on a proposal not in voting period', async () => {
    const proposalId: string = await tGroup.get(1).container.call('createcfp', [{
      title: 'A community fund proposal',
      amount: 100,
      payoutAddress: await tGroup.get(1).container.call('getnewaddress', ['', 'bech32'])
    }])
  ...
  it('should vote with utxos', async () => {
    ...
    const utxos = await tGroup.get(1).container.call('listunspent', [1, 9999999, [masternodes[masternodeId].ownerAuthAddress]])
    const utxo = utxos[0]
    const voteTx = await tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES }, [utxo])
    await tGroup.get(1).container.generate(1)

    expect(typeof voteTx).toStrictEqual('string')
    expect(voteTx.length).toStrictEqual(64)

    const rawtx = await tGroup.get(1).container.call('getrawtransaction', [voteTx, true])
  ...
  it('should not vote with utxos not from the owner', async () => {
    ...
    const utxo = await tGroup.get(1).container.fundAddress(await tGroup.get(1).container.call('getnewaddress'), 10)
    const promise = tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES }, [utxo])
    await expect(promise).rejects.toThrow(RpcApiError)
    await expect(promise).rejects.toThrow(`RpcApiError: 'Test VoteTx execution failed:
tx must have at least one input from the owner', code: -32600, method: vote`)
  ...
  it('should not vote with wrongly formatted utxos\' txid', async () => {
    ...
    const promise = tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES }, [{ txid: 'XXXX', vout: 1 }])
    await expect(promise).rejects.toThrow(RpcApiError)
  ...
  it('should not vote with invalid utxos\' txid', async () => {
    ...
    const promise = tGroup.get(1).rpc.governance.vote({ proposalId, masternodeId, decision: VoteDecision.YES }, [{ txid, vout: 1 }])
    await expect(promise).rejects.toThrow(RpcApiError)
