...
describe('P2WSH', () => {
  ...
  describe('from() - valid address', () => {
    it('should get the type precisely', () => {
      ...
      expect(p2sh.type).toBe('P2WSH')
      expect(p2sh.constructor.name).toBe('P2WSH')
    ...
    it('should work for all recognized network type', () => {
      ...
      expect(testnet.type).toBe('P2WSH')
      expect(testnet.constructor.name).toBe('P2WSH')
      ...
      expect(regtest.type).toBe('P2WSH')
      expect(regtest.constructor.name).toBe('P2WSH')
  ...
  describe('from() - invalid address', () => {
    ...
    it('should be able to validate in address prefix with network', () => {
      ...
      // expect(p2sh.type).toBe('P2WSH') // invalid address guessed type is not promising, as p2wsh and p2sh are versy similar
  ...
  describe('to()', () => {
    it('should be able to build a new address using a script hash (32 bytes, 64 char hex string)', () => {
      ...
      expect(p2wsh.type).toEqual('P2WSH')
    ...
    it('should reject invalid data - not 32 bytes data', () => {
      ...
      try {
        ...
        expect(e.message).toEqual('InvalidScriptHashLength')
      }
    })
  })
  ...
  it('validate()', () => {
    ...
    expect(p2wsh.validatorPassed).toEqual(0)
    expect(p2wsh.valid).toBeFalsy()

    const isValid = p2wsh.validate()
    // expect(p2wsh.validatorPassed).toEqual(4) // length, network prefix, data character set
  ...
  it('guess()', () => {
    ...
    expect(p2wsh.constructor.name).toBe('P2WSH')
    expect(p2wsh.network).toBe(MainNet)
  })
})
