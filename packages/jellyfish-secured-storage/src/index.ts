...
export class ScryptStorage {
  ...
  async encrypt (data: Buffer, passphrase: string): Promise<void> {
    ...
    const key = this._passphraseToKey(passphrase, hash, 64)
  ...
  async decrypt (passphrase: string): Promise<Buffer|null> {
    ...
    const key = this._passphraseToKey(passphrase, data.hash, 64)
  ...
  private _passphraseToKey (passphrase: string, salt: Buffer, keyLength: number): Buffer {
