...
import { ApiError, BadRequestApiException } from '../core/ApiError'
...
export class RawtxController {
  ...
  async send (@Body() tx: RawTxDto): Promise<string> {
    ...
    try {
      ...
      if ((err as ApiError)?.payload?.message === 'TX decode failed') {
        throw new BadRequestApiException('Transaction decode failed')
      }
      if ((err as ApiError)?.payload?.message.indexOf('absurdly-high-fee') !== -1) {
        // message: 'absurdly-high-fee, 100000000 > 11100000 (code 256)'
        throw new BadRequestApiException('Absurdly high fee')
      }

      throw new BadRequestApiException((err as ApiError)?.payload?.message)
    }
  }
  ...
  async test (@Body(ValidationPipe) tx: RawTxDto): Promise<void> {
    ...
    try {
      ...
      if ((err as ApiError).message === 'Transaction is not allowed to be inserted') {
